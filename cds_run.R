...
robyn_object <- paste0(local_path, "/robyn_object.RDS")
...
InputCollect <- robyn_inputs(
  ...
  prophet_country = "TH", # input one country. dt_prophet_holidays includes 59 countries by default
  context_vars = c("Direct", "X.Other."), # e.g. competitors, discount, unemployment etc
  paid_media_spends = c("ShopBack", "Google.Shopping", "Google.PFM", "Facebook...DABA", "Facebook...DPA", "Facebook...BAU", "Facebook...INS", "Google.UAC...INS", "Criteo", "Facebook...Branding" ), # mandatory input
  paid_media_vars = c("ShopBack", "Google.Shopping", "Google.PFM", "Facebook...DABA", "Facebook...DPA", "Facebook...BAU", "Facebook...INS", "Google.UAC...INS",  "Criteo", "Facebook...Branding"), # mandatory.
  # paid_media_vars must have same order as paid_media_spends. Use media exposure metrics like
  # impressions, GRP etc. If not applicable, use spend instead.
  organic_vars = c("SEO"), # marketing activity without media spend
  #factor_vars = c("events"), # specify which variables in context_vars or organic_vars are factorial
  window_start = "2021-01-01",
  window_end = "2022-10-11",
...
hyperparameters <- list(
  Facebook...DABA_alphas = c(0.5, 3),
  Facebook...DABA_gammas = c(0.3, 1),
  Facebook...DABA_thetas = c(0, 0.3),
  Facebook...DPA_alphas = c(0.5, 3),
  Facebook...DPA_gammas = c(0.3, 1),
  Facebook...DPA_thetas = c(0.1, 0.4),
  Google.PFM_alphas = c(0.5, 3),
  Google.PFM_gammas = c(0.3, 1),
  Google.PFM_thetas = c(0.3, 0.8),
  Google.Shopping_alphas = c(0.5, 3),
  Google.Shopping_gammas = c(0.3, 1),
  Google.Shopping_thetas = c(0, 0.3),
  SEO_alphas = c(0.5, 3),
  SEO_gammas = c(0.3, 1),
  SEO_thetas = c(0.1, 0.4),
  ShopBack_alphas = c(0.5, 3),
  ShopBack_gammas = c(0.3, 1),
  ShopBack_thetas = c(0.1, 0.4),
  
  Criteo_alphas = c(0.5, 3),
  Criteo_gammas = c(0.3, 1),
  Criteo_thetas = c(0, 0.3),
  Facebook...BAU_alphas = c(0.5, 3),
  Facebook...BAU_gammas = c(0.3, 1),
  Facebook...BAU_thetas = c(0.1, 0.4),
  Facebook...Branding_alphas = c(0.5, 3),
  Facebook...Branding_gammas = c(0.3, 1),
  Facebook...Branding_thetas = c(0.1, 0.4),
  
  Facebook...INS_alphas = c(0.5, 3),
  Facebook...INS_gammas = c(0.3, 1),
  Facebook...INS_thetas = c(0, 0.3),

  Google.UAC...INS_alphas = c(0.5, 3),
  Google.UAC...INS_gammas = c(0.3, 1),
  Google.UAC...INS_thetas= c(0.1, 0.4)
...
#### calibrating with experimentation generalization

lift_test <- df_clean %>% filter(DATE>"2021-10-01" & DATE < "2021-11-05")


fb_bau <- lift_test$Facebook...BAU*1.2
fb_daba <- lift_test$Facebook...DABA*1.6
fb_dpa <- lift_test$Facebook...DPA*1.1
fb_branding <- lift_test$Facebook...Branding*2.4
pmax <- lift_test$Google.PFM*3.1
shopping <- lift_test$Google.Shopping*2.7
...
channel = c("Facebook...BAU",  "Facebook...DABA", "Facebook...DPA", "Facebook...Branding" , "Google.PFM", "Google.Shopping"),
#liftStartDate must be within input data range
liftStartDate = as.Date(c("2021-10-01", "2021-10-01", "2021-10-01", "2021-10-01", "2021-10-01", "2021-10-01")),
#liftEndDate must be within input data range
liftEndDate = as.Date(c("2021-11-05", "2021-11-05", "2021-11-05", "2021-11-05", "2021-11-05", "2021-11-05")),
#Provided value must be tested on same campaign level in model and same metric as dep_var_type
liftAbs = c(sum(fb_bau), sum(fb_daba), sum(fb_dpa), sum(fb_branding), sum(pmax), sum(shopping)),
#Spend within experiment: should match within a 10% error your spend on date range for each channel from dt_input
spend = c(sum(lift_test$Facebook...BAU), sum(lift_test$Facebook...DABA), sum(lift_test$Facebook...DPA), sum(lift_test$Facebook...Branding), sum(lift_test$Google.PFM), sum(lift_test$Google.Shopping)),
#Confidence: if frequentist experiment, you may use 1 - pvalue
confidence = c(0.99, 0.99, 0.99, 0.99, 0.99, 0.99),
...
OutputCollect <- robyn_outputs(
  ...
  pareto_fronts = 3,
  calibration_constraint = 0.03, # range c(0.01, 0.1) & default at 0.1
...
print(OutputCollect)
select_model <- "5_144_6" # select one from above
